...
/* TAKEN FROM LIQUID FUN SOURCE STARTER CODE AND MODIFIED SLIGHTLY. USED TO SHADE THE PIXEL IN THE RENDERED IMAGE */
...
void EyeCandyShader::Set1f(const char *name, float var) {
  glUniform1f(Handle(name), var);
}

void EyeCandyShader::Set2f(const char *name, const b2Vec2 &vector_var) {
  glUniform2fv(Handle(name), 1, &vector_var.x);
}

void EyeCandyShader::Set3f(const char *name, const b2Vec3 &vector_var) {
  glUniform3fv(Handle(name), 1, &vector_var.x);
}

void EyeCandyShader::Set4f(const char *name, const b2Vec4 &vector_var) {
  glUniform4fv(Handle(name), 1, &vector_var.x);
}
